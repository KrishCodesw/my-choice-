...
const fallback: FAQ[] = [
  ...
  { _id:'3', question:'CP vs SS bathroom fittings — which is better?',       answer:"CP looks great but tarnishes in humid Mumbai conditions. SS is more durable long-term. We stock both — come see in person.",                                        category:'sanitary'   },
  { _id:'4', question:'What hinge for kitchen cabinets?',                    answer:"Concealed soft-close hinges — Hettich Sensys range. Hidden when closed, silent, built to last decades. Bring cabinet dimensions and we'll advise.",                  category:'hardware'   },
  { _id:'5', question:'Do I need to buy here to use the Workers Network?',   answer:"No — the Workers Network is open to everyone. Submit a request and we'll assign the right verified professional. All coordination through MyChoice.",               category:'general'    },
  { _id:'6', question:'How does the WhatsApp quote system work?',             answer:"Add products to your Quote List, click 'Get Quote on WhatsApp.' It opens WhatsApp with your full list pre-filled. Hit Send — owner replies with pricing.",         category:'general'    },
]
  ...
  return (
    <section id="faq" className="bg-[#F4F4F0] border-t border-[#DDDDD5]">
      ...
      <div className="border-b border-[#DDDDD5] px-6 md:px-10 py-4 flex items-center gap-4">
        <span className="font-fraunces text-[#DDDDD5] text-3xl font-light select-none">08</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 divide-y lg:divide-y-0 lg:divide-x divide-[#DDDDD5]">
        ...
        <div className="lg:col-span-3 divide-y divide-[#DDDDD5]">
          {filtered.length === 0 ? (
            <p className="p-8 font-fraunces italic text-[#8A8A7A] text-xl">No questions in this category yet.</p>
          ) : filtered.map(faq => (
            <div key={faq._id}>
              <button
                ...
                <span className="text-[#3D6B45] flex-shrink-0 mt-1 transition-transform duration-300"
