      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-14">
          ...
          <p className="text-[#8a7d6e] text-sm mt-3 max-w-md leading-relaxed">
            Three sectors. Fifty brands. Add any product to your quote list and
            get pricing on WhatsApp instantly.
          </p>
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20 text-[#8a7d6e]">
            <p className="text-lg font-cormorant italic mb-2">
              No products yet in this sector.
            </p>
            <p className="text-sm">
              Add products via the Sanity Studio dashboard.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-px bg-[#ede7db]">
            {filtered.map((p) => {
              ...
              return (
                <div
                  key={p._id}
                  className="bg-[#f7f3ec] hover:bg-white transition-colors group relative overflow-hidden p-6"
                >
                  {/* New / Featured badges */}
                  <div className="absolute top-3 right-3 flex flex-col gap-1">
                    {p.isNew && (
                      <span className="bg-[#b87333] text-[#f7f3ec] text-[8px] tracking-[1.5px] uppercase px-2 py-0.5 font-semibold">
                        New
                      </span>
                    )}
                    {p.isFeatured && (
                      <span className="bg-[#0f1923] text-[#d4956a] text-[8px] tracking-[1.5px] uppercase px-2 py-0.5 font-semibold">
                        Top Pick
                      </span>
                    )}
                  </div>
                  ...
                  {/* Icon */}
                  <div className="w-full aspect-square bg-[#0f1923] rounded overflow-hidden mb-4 relative">
                    {p.image?.asset?.url ? (
                      <img
                  ...
                  {/* Brand */}
                  <p className="text-[9px] tracking-[2px] uppercase text-[#b87333] font-semibold mb-1.5">
                    {p.brand?.name}
                  </p>

                  {/* Name */}
                  <h3 className="font-playfair text-lg font-semibold text-[#1a1a2e] leading-tight mb-2">
                    {p.name}
                  </h3>

                  {/* Category */}
                  {p.category && (
                    <p className="text-[10px] tracking-[1px] uppercase text-[#8a7d6e] mb-2">
                      {p.category}
                    </p>
                  )}
                  ...
                  {/* Spec */}
                  {p.spec && (
                    <p className="text-xs text-[#8a7d6e] leading-relaxed mb-5 line-clamp-2">
                      {p.spec}
                    </p>
                  )}
