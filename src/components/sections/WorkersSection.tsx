...
export function WorkersSection({ workers }: Props) {
  ...
  const handleRequest = () => {
    if (!name.trim() || !phone.trim()) {
      alert('Please enter your name and WhatsApp number so we can confirm your booking.')
      return
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-[#DDDDD5]">
        ...
        <div>
          ...
          <div className="border-t border-[#DDDDD5] px-6 md:px-10 py-6">
            <p className="text-sm text-[#8A8A7A] leading-relaxed max-w-sm">
              Worker contact details are never shared directly. All coordination goes through MyChoice — ensuring quality and full accountability on every job.
            </p>
          </div>
        </div>
        ...
        <div className="p-6 md:p-10 flex flex-col gap-4">
          <div>
            ...
            <p className="text-sm text-[#8A8A7A]">Fill in the details — we'll assign the right professional and confirm via WhatsApp.</p>
          </div>
