...
export function WorkersSection({ workers }: Props) {
  ...
  const handleRequest = () => {
    if (!name.trim() || !phone.trim()) {
      alert(
        "Please enter your name and WhatsApp number so we can confirm your booking.",
      );
      return;
    }
    openWhatsApp(
      waMessages.workerRequest({
  ...
  return (
    ...
    >
      ...
      <div className="border-b border-[#DDDDD5] px-6 md:px-10 py-4 flex items-center justify-between">
        ...
        <span className="label-sm text-[#AEAE9E]">
          All contact via MyChoice only
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-[#DDDDD5]">
              ...
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-1 flex-wrap">
                    <h3 className="font-fraunces text-[#1C2B1A] text-xl">
                      {w.name}
                    </h3>
                    <span className="label-sm text-[#3D6B45] border border-[#3D6B45]/30 bg-[#EBF2EC] px-2 py-0.5">
                      ✓ Verified
                    </span>
                  </div>
                  <p className="label-sm text-[#3D6B45]">
                    {w.specialty
                      ? specialtyMeta[w.specialty]?.label
                      : "Specialist"}
                ...
                {/* <span className="text-[#DDDDD5] flex-shrink-0 text-lg">🔒</span> */}
              </div>
            ))}
          </div>
          <div className="border-t border-[#DDDDD5] px-6 md:px-10 py-6">
            <p className="text-sm text-[#8A8A7A] leading-relaxed max-w-sm">
              Worker contact details are never shared directly. All coordination
              goes through MyChoice — ensuring quality and full accountability
              on every job.
            </p>
          </div>
        </div>
        ...
        <div className="p-6 md:p-10 flex flex-col gap-4">
          <div>
            <p className="font-fraunces text-[#1C2B1A] text-2xl mb-1">
              Request a Worker
            </p>
            <p className="text-sm text-[#8A8A7A]">
              Fill in the details — we'll assign the right professional and
              confirm via WhatsApp.
            </p>
          </div>

          {/* Name + Phone — new fields */}
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className={labelClass}>Your WhatsApp *</label>
              <input
                type="tel"
          ...
          <p className="text-center label-sm text-[#AEAE9E]">
            We'll confirm the worker and date via WhatsApp
          </p>
        </div>
      </div>
    </section>
