...
import { Store, Video, MessageSquare, ArrowRight } from "lucide-react";
...
export function BookingSection() {
  ...
  const [meetingType, setMeetingType] = useState<"showroom" | "video">(
    "showroom",
  );
  ...
  const handleBook = () =>
    openWhatsApp(
      waMessages.bookVisit({
        name: form.name || "[Not provided]",
        phone: form.phone || "[Not provided]",
        date: form.date || "[Not selected]",
        time: form.time || "[Not selected]",
        type:
          meetingType === "showroom" ? "🏪 Visit Showroom" : "📱 Video Call",
        desc: form.desc || "[Not specified]",
      }),
    );

  const inputClass =
    "w-full bg-[#EEEEE8] border border-[#DDDDD5] text-[#1C2B1A] px-4 py-3 text-sm font-chivo focus:border-[#3D6B45] outline-none transition-colors placeholder:text-[#AEAE9E]";
    ...
    <section id="booking" className="bg-[#EEEEE8] border-t border-[#DDDDD5]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-[#DDDDD5]">
        ...
        <div className="px-6 md:px-10 py-12 flex flex-col gap-8 relative overflow-hidden">
          <div className="absolute right-6 bottom-6 font-fraunces text-[#1C2B1A]/4 leading-none select-none pointer-events-none text-[160px]">
            1:1
          </div>
          <div>
            ...
            <p className="text-sm text-[#8A8A7A] leading-relaxed max-w-sm">
              The owner will walk you through it — in the showroom or on a call.
              Bring your floor plan, photos, or just a rough idea.
            </p>
          </div>
            ...
            {[
              {
                ...
                desc: "See products first-hand. 30 min session with the owner.",
              },
              {
                icon: Video,
                title: "Video Call",
                desc: "Quick consultation from wherever you are.",
              },
              {
                icon: MessageSquare,
                title: "Confirmed on WhatsApp",
                desc: "Owner confirms your slot directly. No app needed.",
              },
            ].map((b) => {
              const Icon = b.icon;
              return (
                <div key={b.title} className="flex gap-4 items-start">
                  <Icon
                    className="w-5 h-5 text-[#3D6B45] shrink-0 mt-0.5"
                    strokeWidth={1.5}
                  />
                  <div>
                    ...
                    <p className="text-xs text-[#8A8A7A] mt-0.5 leading-relaxed">
                      {b.desc}
                    </p>
                  </div>
                </div>
        ...
        <div className="px-6 md:px-10 py-12 flex flex-col gap-5">
          ...
          <div>
            ...
            <textarea
              rows={3}
              placeholder="e.g. Electrical for 3BHK, sanitary for 2 bathrooms..."
          ...
          <p className="text-center label-sm text-[#AEAE9E]">
